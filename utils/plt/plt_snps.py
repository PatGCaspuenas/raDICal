...
def plot_video_snp_lic(grid, D, path_out, limits=[-0.5, 0.5], make_axis_visible=[1, 1], show_title=1, show_colorbar=1,
                   flag_flow='SC', flag_control=0, u=[], t=[]):
    import lic
    ...
    M = np.reshape(get_2Dvorticity(grid, D), (m, n, nt), order='F')

    U = np.zeros((size, size, nt, k))
    for i in range(k):
        for it in range(nt):
             Z = np.reshape(D[( (n * m)*i ):( (n * m)*(i + 1) ), it], (m, n), order='F')
             U[:, :, it, i] = griddata(points, Z.flatten(), (X0, Y0))
    ...
    LIC = lic.lic(U[:, :, 0, 0].T, U[:, :, 0, 1].T, length=30)
    ...
    cp000 = ax.contourf(X, Y, B, colors='k')

    def animate(it):

        ax.cla()
        LIC = lic.lic(U[:, :, it, 0].T, U[:, :, it, 1].T, length=30)
